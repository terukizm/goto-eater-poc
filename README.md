...
- 競合サービス
  - https://go-to-eat-map.com/
    - googlemapからleafletに変わってて草、お高いもんな…
...
- [x] 飲食店のジャンル(カテゴリ)ごとに別アイコンを表示させる
  - [x] レイヤーを分けてon/offの切り替えができるようにする
  - [x] 飲食店ジャンル別にGeoJSON用意
- [ ] GitHub Pagesでpublish
...
- [ ] 初期表示時に位置情報を取得し、現在地を中心として地図を描画(現在はハードコーディングで決め打ち)
...
- 指定された場所にマップの中心を移動させる(入力フォームに自治体名、住所を入れるなど)
  - 現状マップ中心を移動させる手段が手動操作しかないので、概ね場所がわかってないと操作できない
    ...
    - 住所からlatlngを取るので(フォワード)ジオコーディングが必要だが精度は不要、[これ](https://github.com/geolonia/community-geocoder)叩かせてもらうのがよさそう
  - とりあえずクエリパラメータでやっつけ動作確認完了
- metaタグとかOGPとかそのへんいい感じに
- [x] Debug用GeoJSONを分ける(with indent, debug情報系props)
...
　- UI側でよしなにできなかったらこれもうしゃーないやろって気もしてきた
    - LatLngに限らずマーカー画像が重なる可能性はあり、その場合はLayer優先度でレンダリングされる以上のことはできないっぽい
    - 'text-variable-anchor'でラベルを出し分けする以上の対応は難しそう
...
- 見ないふりしてる
  ...
  - 手元のMac版SafariとFirefoxで右側に表示されてる現在置ボタンが動かない(Chromeは動く)
    ...
    - この辺MapBox GL LSもしくはGeolonia APIとの切り分けしないとわからん
...
- [x] 飲食店のジャンルは都道府県でまったく統一されてない。ひどい自治体だと25ジャンル以上ありそう
  - [x] 根性で振り分けた
...
- [pass] どうも公式サイトあたり見てると自治体の人が手入力で住所入れてる感じで住所あたりの表記ブレがきつめ
  - 番地表記とかもめちゃくちゃ適当な感じする、申請の値コピペでそのまま入れてない…？
  - 特に大阪,京都がひどいが、どうしようもない
- [x] ジオコーディング失敗時の検出とエラー処理
  - [x] 取れなかったら取れなかったでしゃーない(精度は重視してない)でいいが、ログくらいは出して追いかけられるようにはしたい
    - そのケースに対応するかは別の話として
...
* 有償系
  ...
  * Mapbox
    * 日本だと精度が微妙
  * Yahooのやつ
    * ジオコーディング自体はサー終じゃなかったが、利用規約でNG。あと1日50000回の制限もつらめ
* 無料系
  * 国土地理院地図の内部API
  ...
  * IMIコンポーネントツール・住所変換コンポーネント(community-geocoder)
    ...
    * 実装がDAMSとは違うベクトルできつい、アドホックにゴリゴリ組んでる感じ。
      * アプローチとしてはDAMSの方が知的。テストも書いてあるしこっちのほうがわかりやすくはあるが…
        * 現時点では利用用途が限られすぎる
    * 住所からざっくり地図の中心点を表示させるのにはcommunity-geocoderを使わせてもらうことにした
